...
def test_get_close_db(app):
    """Testing that within an application context, get_db should return the same connection each time it’s called.
...
def test_init_db_command(runner, monkeypatch):
    """This test uses Pytest’s monkeypatch fixture to replace the init_db function with one that records that it’s been called.
     Thia runner fixture is used to call the init-db command by name."""
